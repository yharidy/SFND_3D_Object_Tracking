...
int main(int argc, const char *argv[])
{
    ...
    for (size_t imgIndex = 0; imgIndex <= imgEndIndex - imgStartIndex; imgIndex++)
    {
        ...
        //// STUDENT ASSIGNMENT
        //// TASK MP.2 -> add the followi ng keypoint detectors in file matching2D.cpp and enable string-based selection based on detectorType
        ...
        for (int i = 0; i < keypoints.size(); i++)
        {
            standardDeviation += (keypoints[i].size - mean) * (keypoints[i].size - mean);
        }
        standardDeviation /= keypoints.size();
        
        cout << "Standard deviation of neighborhood size = " << standardDeviation << endl;
        ...
        if (dataBuffer.size() > 1) // wait until at least two images have been processed
        {
            ...
            cout << "Found " << matches.size() << "in " << t << " ms" << endl;
